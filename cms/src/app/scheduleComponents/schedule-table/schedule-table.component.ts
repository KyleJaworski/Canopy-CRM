...
export class ScheduleTableComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(
      this.stateService.selectedDate$.subscribe((date) => {
  ...
  filterMeetingsByMonth(): void {
    ...
    this.stateService.setMeetingsInMonth(this.meetingsInSelectedMonth);
  }

  // Set the selected date and update day-specific meetings

  setSelectedDate(day: { date: string }): void {
    this.stateService.setSelectedDate(day.date);
  }
